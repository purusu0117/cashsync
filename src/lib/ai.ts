...
function receiptPromptBody(categoryNames: string[], today: string): string {
  return [
    "・kind: お金を【払った/送った】画面なら expense、お金を【受け取った】画面なら income。PayPay/LINE Pay 等の送金画面は、必ず次の2点だけで判断する（他の文言に惑わされない）:",
    "  - ①一番上のタイトル: 『◯◯さんに送る／◯◯へ送る／◯◯へ送金』＝自分が送った＝expense。 『◯◯さんから受け取る／◯◯から受け取る』＝自分が受け取った＝income。助詞『に/へ＝送る側=expense』『から＝受け取る側=income』が決め手。",
    "  - ②金額の先頭の符号: 『＋』が付く（＋1,450円）＝income。符号が無い（300円）＝expense。",
    "  - 【重要な罠】緑の『受け取り完了』というバッジは、送った画面（expense）にも受け取った画面（income）にも両方表示される。だから『受け取り完了』の文字だけで income と判断してはいけない。必ず①タイトルの『送る/受け取る』と②金額の＋の有無で決める。",
    "  - その他の income: 給与振込・フリマの売上・『入金』。その他の expense: レシート・支払い完了・注文確認・『お支払い』。",
