...
export async function GET() {
  try {
    ...
    return Response.json({ month, pockets: pocketBudgets(user.id, month) });
  } catch (e) {
  ...
  try {
    ...
    if (amount === 0) {
      db()
        ...
        .run(user.id, body.categoryId);
    } else {
      const carryover = body.carryover ? 1 : 0;
      db()
        .prepare(
        ...
        .run(user.id, body.categoryId, amount, carryover);
    }
