...
export async function GET(request: Request) {
  try {
    ...
    const d = await db();
    const shifts = await d.all<ShiftRow & { source: string; job_name: string; job_color: string }>(
    ...
    return Response.json({ shifts: withPay, income: await monthWorkIncome(user.id, month) });
  } catch (e) {
...
export async function DELETE(request: Request) {
  try {
    ...
    const d = await db();
    await d.run("DELETE FROM shifts WHERE id = ? AND user_id = ?", id, user.id);
