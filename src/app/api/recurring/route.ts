...
export async function POST(request: Request) {
  try {
    ...
    db()
      ...
      .run(id, user.id, kind, name, amount, body.categoryId ?? null, startMonth, endMonth, postDay, interval, isFixed);
...
export async function PUT(request: Request) {
  try {
    ...
    db()
      .prepare("UPDATE recurring_items SET is_fixed = ? WHERE id = ? AND user_id = ?")
      .run(body.isFixed ? 1 : 0, body.id, user.id);
