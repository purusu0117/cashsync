...
export async function GET(request: Request) {
  try {
    ...
    if (params.get("search") === "1") {
      const result = await searchExpenses(user.id, {
        q: params.get("q") ?? "",
...
export async function PUT(request: Request) {
  try {
    ...
    // 横断タグは任意。tagIds が配列で来た時だけ張り替える（未指定は expense_tags を一切触らない）
    if (Array.isArray(body.tagIds)) await setExpenseTags(user.id, body.id, body.tagIds);
...
export async function DELETE(request: Request) {
  try {
    ...
    // 横断タグの紐付けも掃除（孤児行を残さない。joinで無視されるため集計には元々影響しない）
    await d.run("DELETE FROM expense_tags WHERE expense_id = ?", id);
    return Response.json({ ok: true });
  } catch (e) {
    if (e instanceof AuthError) return unauthorized();
    return Response.json({ error: String(e) }, { status: 500 });
