// CashSync 動画パイロット用シード — 架空ユーザー「タロー」。APIのみ経由（本名は一切使わない）。
// 使い方: CASHSYNC_BASE=http://localhost:3017 node .demo/vid1-seed.mjs
...
// --- 3) 貯金目標 ---
await post("/api/profile", { savingsGoal: 30000 });
...
// --- 5) 定期収入・定期支出（黒字ペースになるバランスに調整） ---
await post("/api/recurring", { kind: "income", name: "給料", amount: 245000, postDay: 25, startMonth: MONTH });
await post("/api/recurring", {
  kind: "expense", name: "家賃", amount: 62000, postDay: 27, isFixed: true,
...
// --- 7) 当月の支出（自然なメモ・一部にタグ）。7/28より前の日付のみ＝今日の分は動画内で記録する ---
const E = [
  ["2026-07-01", 3200, "食費", "スーパー まとめ買い"],
  ["2026-07-02", 680, "食費", "ランチ"],
  ["2026-07-03", 1500, "交通", "Suicaチャージ"],
  ["2026-07-05", 2300, "娯楽", "ライブ物販", [tOshi]],
  ["2026-07-07", 2980, "洋服", "Tシャツ"],
  ["2026-07-09", 1500, "交際", "友達とランチ"],
...
console.log("=== 検証 ===");
console.log("支出:", exp.expenses.length, "件");
console.log("allowance(今日あと使える):", sum.allowance);
console.log("forecast(月末予測):", sum.forecast?.forecast, "貯金目標:", sum.savingsGoal);
